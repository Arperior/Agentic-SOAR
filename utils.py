...
def generate_oof_features(
    ...
    rcf_num_trees=80, rcf_tree_size=256,
    global_p1=None, global_p99=None, global_p995=None
):
    """
    ...
    - RCF receives raw-scaled features (no PCA) to preserve anomaly geometry.
    - CatBoost and meta-learner receive PCA-transformed features as before.
    """
    ...
    X_num_raw_reset = X_num_raw.reset_index(drop=True)
    ...
    for train_idx, val_idx in skf.split(X_cat_reset, stratify_labels):
        ...
        # ── PCA path (CatBoost + meta-learner) ───────────────────────────
        # Fit scaler + PCA exclusively on this fold's training split.
        # Validation slice is only .transform()'d — never touches .fit().
        ...
        # ── Raw-scaled path (RCF only) ────────────────────────────────────
        # RCF needs the original numeric space to detect geometric anomalies.
        # PCA rotates and compresses the space, which destroys the sparse
        # local structure that isolation trees rely on.
        fold_rcf_scaler = RobustScaler()
        X_num_raw_tr_attack = X_num_raw_tr[y_tr.values == 1]
        fold_rcf_scaler.fit(X_num_raw_tr_attack)
        X_rcf_tr_attack = fold_rcf_scaler.transform(X_num_raw_tr_attack)
        X_rcf_val       = fold_rcf_scaler.transform(X_num_raw_val)
 
        # 1. CatBoost binary score
        temp_cat = train_cat_func(X_cat_tr, y_tr, cat_cols)
        oof_cat[val_idx] = temp_cat.predict_proba(X_cat_val)[:, 1]
 
        # 2. RCF anomaly score — trained on attack-only raw-scaled features
        temp_rcf = rcf_class(num_trees=rcf_num_trees, tree_size=rcf_tree_size)
        temp_rcf.fit_predict(X_rcf_tr_attack, global_p1=global_p1, global_p99=global_p99, global_p995=global_p995)
        oof_rcf[val_idx] = 1.0 - temp_rcf.predict_proba(X_rcf_val) 
